...
const NewInvoice = () => {
  ...
  const saveData = async () => {
    ...
    try {
      ...
      // Save the invoice data to Firestore along with the PDF URL
  ...
  const generatePDFAndUpload = async () => {
    ...
    // Load company logo and owner's signature
    const companyLogoURL = document.getElementById('companyLogo').files[0] ? await uploadFile(document.getElementById('companyLogo').files[0], 'companyLogos') : '';
    const ownerSignURL = document.getElementById('ownerSign').files[0] ? await uploadFile(document.getElementById('ownerSign').files[0], 'ownerSignatures') : '';
  
    // Add company logo
    if (companyLogoURL) {
      const logoImage = await fetch(companyLogoURL).then(res => res.blob());
      const logoImageData = URL.createObjectURL(logoImage);
      pdf.addImage(logoImageData, 'PNG', 10, 10, 50, 50); // Adjust position and size as needed
    }
  
    // Add owner's signature
    if (ownerSignURL) {
      const signImage = await fetch(ownerSignURL).then(res => res.blob());
      const signImageData = URL.createObjectURL(signImage);
      pdf.addImage(signImageData, 'PNG', pdfWidth - 60, pdfHeight - 60, 50, 50); // Adjust position and size as needed
    }
